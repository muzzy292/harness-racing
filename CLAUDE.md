...
### Scoring / Weights
- **One-hit-wonder overrating via sp_trend — uncapped, dominates score on large shortenings** — `_pos_scale(sp_trend, center=0.0, divisor=0.05)` has no upper cap. A 37pp shift in implied probability (e.g. $51→$9→$2.30) produces a raw score of 7.4 × weight 0.3 = **+2.22**, making it the second-largest S1 component regardless of the horse's actual form. Two confirmed examples: (1) SEASIDE SID (TM060426 R6): won last start at $2.10 vs NR56 field, model $4.18 vs $6 market, finished 7th by 33.9m. `ceiling_support_rate = 0.2`, `avg_adj_margin = 8.12m`. (2) TASBMIKI (BH080426 R2): prior SPs $51/$9/$19, $2.30 last start — model $2.28 vs $11 SP, finished 4th; `sp_trend = 0.371` → +2.22 S1 contribution. The grade-discount on `last_prob` (committed d1b6907) partially addresses grade-drop inflation but not the magnitude. Possible fixes: (A) cap the raw scale value (e.g. max 3.0 → max score +0.90); (B) dampen weight when shortening is concentrated in a single run vs a sustained 2–3 start trend; (C) require minimum n prior SPs before the trend fires. Collecting more examples before changing.
...
- **`sp_class` over-penalises grade-drop horses (STUDLEIGH MELISE — BH080426 R2)** — NR68 horse dropping to NR58 field won at $3.40; model had $16.12. Being a $21 outsider in NR63 races is expected for a horse of that class, not a signal of poor ability relative to today's weaker field. `sp_class` drag (-0.600) combined with `market` (-0.913) and `sp_trend` (-0.810) = -3.51 total, swamping grade-drop signals (`nr` + `class_pos` + `nr_grade_delta` = +1.12). Proposed fix: suppress/dampen `sp_class` when `nr_headroom < -5` OR increase `nr_grade_delta` signal strength (currently divisor=-10.0, weight=0.4 = only +0.04 per NR point). Needs more examples before changing weights.
