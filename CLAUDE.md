...
### Data Quality
- **`runner_recent_lines` re-ingest — RESOLVED (idempotent).** Re-ingesting the same meeting does NOT create duplicates: `runner_recent_lines` has `PRIMARY KEY (meeting_code, race_number, horse_id, line_index)` and the insert uses `ON CONFLICT(...) DO UPDATE` (upsert), so re-ingesting overwrites the same rows in place. This is what makes the race-day `refresh-today` step safe. Note: the same underlying run appears under multiple `meeting_code`s (e.g. BH010426 and BH080426) **by design** — each meeting keeps a per-meeting form snapshot so `features.py` can filter `run_date < meeting_date` for leak-free historical scoring. That is intentional, not bloat; do NOT add a cross-meeting unique constraint (it would break the snapshot/leak-free filtering).
