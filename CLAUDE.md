...
- **Barrier relief signal — ready to implement** — horses drawing a better barrier than their recent average should get a bonus beyond the static barrier score. Historical barrier data is already stored in `runner_recent_lines.mile_rate` (stores "Fr7"-type strings from HTML form lines). Plan: add `_barrier_num_score()` helper in `features.py` (mirrors `_barrier_score()` in `odds.py`), compute `barrier_relief_score = today_barrier_score - avg(recent_barrier_scores[:5])`, clip at 0 (only reward improvement — static score handles worsening), add to S2 at weight 0.4. Trigger example: CLASSIC MISSION NZ (PC110426 R3). Full implementation plan in `.claude/plans/encapsulated-herding-conway.md`.

### Scoring / Weights
- **Remove all SP signals from S1 — consider full removal** — `sp_trend`, `sp_class`, and `market` (avg SP) are all derived from historical starting prices stored in `runner_recent_lines`. Across multiple investigations they have proven to be the most distorting components in the model: `sp_trend` overrates horses that shortened last start and underrates horses that drifted (TASBMIKI, SEASIDE SID, ROCKET ME); `sp_class` penalises grade-drop horses, maiden runners, and any horse whose past SP doesn't reflect today's field (STUDLEIGH MELISE, CAPTAINS DELIGHT); `market` (avg SP) compounds these by treating long-shot history as a persistent ability deficit. Combined, these three components can produce swings of 3–5 S1 points that overwhelm genuine form signals like margins, win rate, and sectionals. The model is supposed to be an independent form assessment — using past market prices as inputs creates circular dependency and anchors the model to stale market opinions. Proposed approach: zero out `sp_trend`, `sp_class`, and `market` weights (set to 0 in `weights.json`) and run against a known meeting to assess whether raw form signals alone produce better differentiation. The `recent_line_last_sp` and `recent_line_sp_class_score` columns remain in the CSV so the signals can be reinstated if removal makes things worse.
- **`sp_trend` uncapped — overrates large shortenings AND underrates large drifters** — `_pos_scale(sp_trend, center=0.0, divisor=0.05)` has no cap in either direction. Works as a two-way distortion: a single atypical SP (one blow-out or one short-price run) dominates S1 regardless of the horse's actual form. Confirmed examples in both directions:
  - **(Overrating — shortening)** SEASIDE SID (TM060426 R6): model $4.18 vs $6 SP, finished 7th by 33.9m. TASBMIKI (BH080426 R2): prior SPs $51/$9/$19, $2.30 last start — model $2.28 vs $11 SP; `sp_trend = 0.371` → **+2.22 S1 contribution**.
  - **(Underrating — drifting)** ROCKET ME (YU100426 R1): ran at $23 last start (drifted from ~$3), model $56.75 vs SP $3.50. `sp_trend = -0.291` → **-1.75 S1 contribution**. Market had re-backed the horse to $3.50 next start, but model was anchored on the drift. Horse also affected by class-adj inflation (4.6m actual avg loss → 12.1m class-adj, ran in NR42 races vs NR55 ceiling) and GEM N EM NZ dominating softmax (5/5 wins).
  - Possible fixes: (A) cap the raw scale value (e.g. ±3.0 → max ±0.90 contribution); (B) dampen weight when shift is concentrated in a single run vs a sustained 2–3 start trend; (C) require minimum n prior SPs before the trend fires. Collecting more examples before changing.
...
- **`dist_strike_rate` is penalty-only — misses genuine distance specialists** — current design penalises poor distance records but gives zero credit when ratio > 1. SKY CITY NZ (NR100426 R8): model $54.82 vs SP $8. `dist_strike_rate_ratio = 2.48` over 20 distance starts (wins at 2.48× career rate) — contributes nothing to S2. Model favourite OPEN THE GATES had 1 distance start. The penalty-only design was intentional to avoid overrating horses with high ratio but poor overall form (e.g. HEEZ REASONABLE: ratio 4.67, n=9, model $1081 — correctly outsider). The gap: when a horse has reasonable overall form AND a strong distance record (n ≥ 15), the signal is being ignored. Proposed fix: add a capped positive component when `dist_rge_starts >= 15` and `ratio > 1.0`, e.g. `min((ratio - 1.0) * 0.3, 0.6)` to cap the boost. Needs more examples before changing.
...
### Track Pars
- **`_sectional_deltas_vs_par` applies `tempo_adjustment` in the wrong direction** — `features.py:634` does `adjusted_last_half = last_half + tempo_adjustment`. Since `tempo_adjustment = -1.5` for slow-tempo races, this makes the last_half appear **1.5s faster**, inflating the sectional delta and boosting `late_speed`. Example: TASBMIKI (BH080426 R2) had 2/3 last runs tempo-adjusted: raw deltas were -0.2, -2.0, -0.2 (avg -0.8) but the model computed -1.7, -2.0, -1.7 (avg **-1.8**), pushing `late_speed` from +0.93 to **+2.10**. The `tempo_adj` and `tempo_flags` S1 components already penalise slow-tempo runs; the `tempo_adjustment` was designed for margin normalisation, not sectionals. Fix: remove the `tempo_adjustment` application from `_sectional_deltas_vs_par` and use the raw `last_half` directly. Not changing yet as this was a deliberate design choice at model creation — needs discussion.
...
- **Track name mismatch — `'Bathurst'` vs `'Bthurst'` in track_pars** — `meetings` table stores `track_name = 'Bathurst'` (from HTML title); `track_pars.json` key is `'Bthurst'` (from sectionals parser). Result: `race_par_last_half` is None for all BH meetings in the features CSV. `late_speed` scoring is unaffected (uses per-run pars from `runner_recent_lines.track_name = 'Bthurst'`). Fix: normalise track name in `parsers.py` or add a `'Bathurst'` alias in `track_pars.json`.
