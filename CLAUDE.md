...
### Scoring / Weights
- **One-hit-wonder overrating via sp_trend** — a single grade-drop win generates a large shortening signal that can dominate the score even when overall consistency is poor. SEASIDE SID (TM060426 R6): won last start at $2.10 (NR52 vs NR56 field today), model $4.18 vs $6 market, finished 7th by 33.9m. `ceiling_support_rate = 0.2`, `avg_adj_margin = 8.12m`. The grade-discount on `last_prob` (committed d1b6907) partially addresses this but doesn't fully resolve. Possible further fix: dampen `sp_trend` weight when shortening is concentrated in a single run vs a sustained 2–3 start trend. Needs more examples before changing weights.
