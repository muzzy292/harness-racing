...
def _stage2_components(row: dict[str, str]) -> dict[str, float]:
    ...
    """
    map_lead = _to_float(row.get("map_leader_pos_score"))
    map_death = _to_float(row.get("map_outside_leader_pos_score"))
    map_soft_pegs = _to_float(row.get("map_soft_pegs_score"))
    lead_probability = _to_float(row.get("lead_probability")) or 0.0
    leaders_back_probability = _to_float(row.get("leaders_back_probability")) or 0.0
    parked_probability = _to_float(row.get("parked_probability")) or 0.0
    one_one_probability = _to_float(row.get("one_one_probability")) or 0.0
    three_back_pegs_probability = _to_float(row.get("three_back_pegs_probability")) or 0.0
    one_out_two_back_probability = _to_float(row.get("one_out_two_back_probability")) or 0.0
    back_pegs_probability = _to_float(row.get("back_pegs_probability")) or 0.0
    one_out_back_probability = _to_float(row.get("one_out_back_probability")) or 0.0
    three_wide_risk_probability = _to_float(row.get("three_wide_risk_probability")) or 0.0
    leader_candidate_rank = _to_float(row.get("leader_candidate_rank"))
    pace_pressure_score = _to_float(row.get("pace_pressure_score")) or 0.0
    map_confidence = _to_float(row.get("predicted_map_confidence")) or 0.25
    ...
    confidence_scale = max(0.2, min(0.85, map_confidence))
    pressure_leader_boost = max(0.35, 1.0 - (pace_pressure_score * 0.75))
    pressure_soft_boost = 0.9 + (pace_pressure_score * 0.2)
    pressure_back_relief = pace_pressure_score * 0.12
    leader_rank_penalty = max(0.0, ((leader_candidate_rank or 1.0) - 1.0) * 0.04)

    return {
        "barrier":      _barrier_score(barrier),
        "map_lead":     ((lead_probability * 0.7) + ((map_lead or 0.0) * 0.08) - leader_rank_penalty) * 0.30 * confidence_scale * pressure_leader_boost,
        "map_soft_pegs": ((leaders_back_probability + (three_back_pegs_probability * 0.75)) * 0.26 + ((map_soft_pegs or 0.0) * 0.05)) * confidence_scale * pressure_soft_boost,
        "map_soft":     (one_one_probability * 0.34) * confidence_scale * pressure_soft_boost,
        "map_back":    -max(
            0.0,
            ((one_out_two_back_probability * 0.55) + (back_pegs_probability * 0.45) + (one_out_back_probability * 0.65))
            - pressure_back_relief,
        ) * 0.24 * confidence_scale,
        "map_death":   -((parked_probability * 0.8) + (three_wide_risk_probability * 0.3) + ((map_death or 0.0) * 0.06)) * 0.28 * confidence_scale * (0.85 + (pace_pressure_score * 0.45)),
        "map_pressure": ((pace_pressure_score * 0.12) * ((back_pegs_probability * 0.8) + (one_out_back_probability * 0.55))) - ((pace_pressure_score * 0.1) * lead_probability),
        "map_uncertainty": -max(0.0, 0.55 - map_confidence) * 0.18,
