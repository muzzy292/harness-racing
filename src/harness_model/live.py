...
function parseTabStatement(text) {
  ...
  for (const raw of text.split(/\n/)) {
    ...
    const returnAmt = m[7] ? parseFloat(m[7]) : null;

    const prefix = _venuePrefix(venueRaw);
    const mc     = `${prefix}${dd}${mm}26`;

    // Use time from statement row for accurate placed_at (AEST +10:00)
    const timeM    = line.match(/\b(\d{2}:\d{2}:\d{2})\b/);
    const time     = timeM ? timeM[1] : '12:00:00';
    const placedAt = `2026-${mm}-${dd}T${time}+10:00`;

    // Stake first — need it to distinguish a win return from a $0 loss return
    const dollars = [...line.matchAll(/\$(\d+\.\d{2})/g)].map(x => parseFloat(x[1]));
    let stake = 1.0;
    if (dollars.length >= 3) stake = dollars[dollars.length - 3];
    if (stake <= 0) stake = 1.0;

    // Result: TAB includes =Return:$0.00 on losses too, so check returnAmt > stake
    const afterPrice = line.slice(line.indexOf('@Price:'));
    let result = 'pending', payout = null;
    if (returnAmt !== null && returnAmt > stake) {
      // Genuine win: return exceeds what was staked
      result = 'won';
      payout = returnAmt;
    } else if (/\bLost\b/i.test(afterPrice) || (returnAmt !== null && returnAmt === 0)) {
      result = 'lost';
    } else if (/\bCredit\b/i.test(afterPrice) || (returnAmt !== null && Math.abs(returnAmt - stake) < 0.01)) {
      result = 'void';   // debit row credited back at cost = refund/void
    }

    if (payout === null) payout = parseFloat((stake * tabOdds).toFixed(2));
