...
function parseTabStatement(text) {
  ...
  for (const raw of text.split(/\n/)) {
    ...
    // Determine result from text after @Price: in the line
    const afterPrice = line.slice(line.indexOf('@Price:'));
    let result = 'pending', payout = null;
    if (returnAmt !== null) {
      result = 'won';
      payout = returnAmt;
    } else if (/\bLost\b/i.test(afterPrice)) {
      result = 'lost';
    } else if (/\bCredit\b/i.test(afterPrice)) {
      result = 'void';   // debit row marked Credit = refund/void
    }

    // Stake: dollar amounts in the line — last 3 are debit / credit / balance
