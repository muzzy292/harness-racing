...
def _build_feature_row(
    ...
    prior_driver = next(
        (line.get("line_driver_name") for line in valid_recent_lines if line.get("line_driver_name")),
        None,
    )
    if prior_driver and runner.get("nominated_driver"):
        driver_change_flag = int(
            not _person_names_match(str(prior_driver), str(runner["nominated_driver"]))
        )
    else:
...
_SECTIONAL_MARGIN_THRESHOLD = 8.0  # metres (adjusted) — runs further off the pace than this
...
# 8m matches the backlog example (TASBMIKI: 8.3m inflated late_speed by ~0.5s).
# Using adjusted_margin so the filter aligns with the margin the scoring model sees.
